...
function MockUserAvatarMenu({ user, loading }: { user?: any; loading?: boolean }) {
  if (loading) {
    return (
      <div
  ...
  const displayName = user ? (user.firstName && user.lastName ? `${user.firstName} ${user.lastName}` : user.email || "User") : "Guest User";
  const email = user?.email ?? null;
  const imageUrl = user?.profilePictureUrl ?? null;
  const initials = user ? (displayName.split(/\s+/).map((part: string) => part[0]).join("").toUpperCase() || "U") : "GU";
...
export const AuthenticatedWithName: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: "Authenticated user with first/last name and email. Shows initials 'JD'.",
      },
    },
  },
};
...
export const AuthenticatedEmailOnly: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: "Authenticated user with only email (no name). Shows first letter of email as initial.",
      },
    },
  },
};