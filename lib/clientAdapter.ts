...
type LogEntry = { level: string; msg: string; meta?: Record<string, any>; time?: string };
...
export function createClientAdapter() {
    ...
    function flush() {
        ...
        fetch('/api/logs', { method: 'POST', headers: { 'content-type': 'application/json' }, body }).catch(() => { });
    }
    ...
    const adapter = {
        trace: (m: string | Error, meta?: Record<string, any>) => {
            queue.push({ level: 'trace', msg: typeof m === 'string' ? m : m.message, meta, time: new Date().toISOString() });
            if (queue.length >= MAX_BATCH) flush(); else scheduleFlush();
        },
        debug: (m: string | Error, meta?: Record<string, any>) => {
            queue.push({ level: 'debug', msg: typeof m === 'string' ? m : m.message, meta, time: new Date().toISOString() });
            if (queue.length >= MAX_BATCH) flush(); else scheduleFlush();
        },
        info: (m: string | Error, meta?: Record<string, any>) => {
            queue.push({ level: 'info', msg: typeof m === 'string' ? m : m.message, meta, time: new Date().toISOString() });
            if (queue.length >= MAX_BATCH) flush(); else scheduleFlush();
        },
        warn: (m: string | Error, meta?: Record<string, any>) => {
            queue.push({ level: 'warn', msg: typeof m === 'string' ? m : m.message, meta, time: new Date().toISOString() });
            if (queue.length >= MAX_BATCH) flush(); else scheduleFlush();
        },
        error: (m: string | Error, meta?: Record<string, any>) => {
            queue.push({ level: 'error', msg: typeof m === 'string' ? m : m.message, meta, time: new Date().toISOString() });
            if (queue.length >= MAX_BATCH) flush(); else scheduleFlush();
        },
